# Анатолию в последний месяц удача улыбалась очень плохо.
# У него 3 раза взломали пароль.
# Вот он и задумался над тем, что неправильно подходит к вопросу составления паролей.
# Чтобы не напрягаться больше и опять не попасть впросак,
# молодой человек решил написать функцию на Python,
# которая будет проверять его пароль на надежность.
# Требования к паролю у Анатолия следующие (он внимательно изучил рекомендации знатоков):

# 1. Длина – 8 символов (если меньше – то проще взломать, а если длиннее – то сложно запомнить).

# 2. В пароле должны быть:
# 1) заглавные буквы,
# 2) строчные символы,
# 3) числа
# 4) специальные знаки (из перечня *-#; другие спецсимволы недопустимы, так как Анатолий их не может запомнить).

# Помогите парню составить функцию check_pass (pswd),
# которая проверит пароль на соответствие требованиям.
# В случае верного пароля выведется на печать Пароль идеален,
# а в остальных случаях будут перечислены все ошибки,
# которые Анатолий допустил
# (для представления перечня ошибок заведите переменную err в виде словаря).
def check_pass(pswd: str):
    err = {
        'Длина': 'Длина не равна 8 символам',
        'Заглавные': 'Заглавные буквы не используются',
        'Строчные': 'Строчные буквы не используются',
        'Числа': 'Числа не используются',
        'Специальные знаки': 'Специальные знаки не используются'
    }

    if len(pswd) != 8:
        return err['Длина']
    elif pswd.islower():
        return err['Заглавные']
    elif pswd.isupper():
        return err['Заглавные']
    elif not any(i.isdigit() for i in pswd):
        return err['Числа']
    elif not ('*' in pswd or '-' in pswd or '#' in pswd):
        return err['Специальные знаки']
    else:
        return 'Пароль идеален'


print(check_pass('Hpw23*k9'))
print(check_pass('хуй'))
